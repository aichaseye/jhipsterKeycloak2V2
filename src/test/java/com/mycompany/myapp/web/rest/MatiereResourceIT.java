...
class MatiereResourceIT {
    ...
    void putWithMissingIdPathParamMatiere() throws Exception {
        ...
        restMatiereMockMvc
            .perform(
                put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(matiere))
            )
    ...
    void partialUpdateMatiereWithPatch() throws Exception {
        ...
        partialUpdatedMatiere.reference(UPDATED_REFERENCE).image(UPDATED_IMAGE).imageContentType(UPDATED_IMAGE_CONTENT_TYPE);
